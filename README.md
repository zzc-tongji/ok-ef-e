...
- 日常任务（送礼、据点兑换、委托转交、领取委托奖励、造装备、收信用、收集线索、买卖货、日常奖励等）
- 运送委托接取（按券种 + 金额区间筛选自动抢单，智能滚动扫描，抢单重试）
- 自动送货（武陵 7.31w 委托，滑索编号识别导航）
- 仓库物品转移（跨仓库批量转运，支持最小保留数量）
- 毕业基质识别（遍历武器基质列表，按 `assets/weapon_data.csv` 识别毕业词条并锁定）
